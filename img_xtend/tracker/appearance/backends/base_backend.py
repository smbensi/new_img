...
from img_xtend.tracker.appearance.backbones import build_model, get_nr_classes
...
class BaseModelBackend:
    ...
    def get_crops(self, xyxys, img):
        ...
        if isinstance(xyxys, np.ndarray) and xyxys.ndim==1:
        ...
        for box in xyxys:
            ...
            try:
                ...
                LOGGER.debug(f"ERROR IN RESIZE:{x1=}, {y1=}, {x2=}, {y2=} ")
                crop = cv2.resize(
                    img,
                    resize_dims,  # from (x, y) to (128, 256) | (w, h)
                    interpolation=interpolation_method,
